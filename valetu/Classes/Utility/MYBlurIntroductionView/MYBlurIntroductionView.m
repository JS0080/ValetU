...
-(void)initializeViewComponents{
    ...
    [self.RightSkipButton setBackgroundImage:[UIImage imageNamed:@"f2Ee.png"] forState:UIControlStateNormal];
...
- (void) didPressNext
{
    ...
    if (self.CurrentPanelIndex == -1) {
        ...
        if (LastPanelIndex != self.CurrentPanelIndex) { //Keeps from making the callback when just bouncing and not actually changing pages
            ...
            [self.LeftSkipButton setTitle:@"" forState:UIControlStateNormal];
            [self.RightSkipButton setBackgroundImage:[UIImage imageNamed:@"f253.png"] forState:UIControlStateNormal];
            [self.RightSkipButton addTarget:self action:@selector(didStartParking) forControlEvents:UIControlEventTouchUpInside];
        }
    }
}

- (void) didStartParking
{
    [self skipIntroduction];
}
